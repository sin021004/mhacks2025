...
class PostureDetector:
    def __init__(self, video_source=0):
        ...
        self.CALIBRATION_FRAMES = 90 # 3 seconds at ~30fps
    ...
    def _processing_loop(self):
        """Main loop that runs in the background thread with auto-calibration."""
        while self.is_running:
            ...
            try:
                ...
                if not self.analyzer.calibrated:
                    ...
                    if self.frame_counter >= self.CALIBRATION_FRAMES:
                        success, message = self.analyzer.calibrate(landmarks)
                        self.posture_reason = message
                else:
            ...
            cv2.putText(image, f'STATUS: {self.current_posture}', (15, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, cv2.LINE_AA)
            cv2.putText(image, self.posture_reason, (10, 60), 
    ...
    def get_current_data(self):
        """Returns the current posture data, including the reason."""
        return {
